	...
	<div class="big_content" style = "margin-bottom:50px">
		...
		<div>
			<c:if test="${dto.prevNum ne 0}">
				<a href="detail.do?num=${dto.prevNum }&condition=${condition}&keyword=${keyword}"><i class="em em-arrow_up_small"></i>이전글</a> |
			</c:if>
			<c:if test="${dto.nextNum ne 0}">
				<a href="detail.do?num=${dto.nextNum }&condition=${condition}&keyword=${keyword}" style = "margin-bottom:30px"><i class="em em-arrow_down"></i></i>다음글</a>
			</c:if>
		</div>
		
		<c:if test="${sessionScope.id eq dto.writer }">
			<br />
			<a href="private/updateform.do?num=${dto.num }" class="btn btn-primary">수정</a>
